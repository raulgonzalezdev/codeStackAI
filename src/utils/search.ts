    ...
    if (results.length === 0) {
      ...
      try {
        ...
        if (response && response.choices && response.choices.length > 0) {
          ...
          updateStatusBarMessage(`codeStackAI: loading results`, 2000);
          return { results: [{ sourceURL: 'codeStackAI', code }] };
        }
      } catch (error: any) {
        throw new Error(`Error searching with ChatGPT: ${error.message}`);
      }
      updateStatusBarMessage(`codeStackAI: Start loading snippet results...`, 2000);
    }

    cachedResults[keyword] = results;
    updateStatusBarMessage(`codeStackAI: Finished loading ${results.length} results`);
