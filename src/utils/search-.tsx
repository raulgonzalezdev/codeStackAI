...
export async function search(keyword: string): Promise<null | { results: SnippetResult[] }> {
    ...
    const promise = new Promise<{ results: SnippetResult[] }>(async (resolve, reject) => {
        ...
        vscode.window.setStatusBarMessage(`codeStackAI: Finished loading ${results.length} results`);
    });

    vscode.window.setStatusBarMessage(`codeStackAI: Start loading snippet results...`, promise);
    return promise;
}
